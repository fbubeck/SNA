...
## Data Visualization
Um die Degrees für die Visualisierung nutzen zu können, müssen diese zuvor in ein Dataframe umgewandelt werden. Dies geschieht mit der Funktion \textit{as.data.frame}. Anschließend wird die Library \textit{ggplot2} für das Histogramm angewendet. 
...
```{r fig.cap = "Netzwerk Visualisierung 1"}
# Data Viz for Subset
# network diagramm
ggraph(net_exp, layout = 'fr', maxiter = 100) + 
  ...
  theme_graph()
```
```{r fig.cap = "Netzwerk Visualisierung 2"}
ggraph(net_exp, layout = 'kk', maxiter = 100) + 
  geom_node_point(colour="#e2001a") + 
  geom_edge_link(alpha = .4) +
  theme_graph()
```
```{r fig.cap = "Netzwerk Visualisierung 3"}
# coord diagramm
