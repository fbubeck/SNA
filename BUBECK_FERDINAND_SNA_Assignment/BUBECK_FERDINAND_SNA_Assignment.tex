...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Degree Centrality}
\NormalTok{centr_degree <-}\StringTok{ }\KeywordTok{degree}\NormalTok{(net_exp)}

\NormalTok{df_centr_degree <-}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(centr_degree)}

\NormalTok{top_}\DecValTok{5}\NormalTok{_degree <-}\StringTok{ }\NormalTok{df_centr_degree }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{top_n}\NormalTok{(}\DecValTok{5}\NormalTok{) }\OperatorTok{%>%}\StringTok{  }\CommentTok{# highest values}
\StringTok{  }\KeywordTok{arrange}\NormalTok{(}\OperatorTok{-}\NormalTok{centr_degree)}

\NormalTok{top_}\DecValTok{5}\NormalTok{_degree}
\end{Highlighting}
\end{Shaded}
...
Zu sehen sind die top 5 Gradwerte der Teilmenge des Datensatzes. Dies
bedeutet, dass Knoten Nummer 9 der Knoten ist, welcher die meisten
Kanten zu anderen Knoten hat. In dieser Teilmenge des Datensatzes ist
...
\NormalTok{top_}\DecValTok{5}\NormalTok{_betweenness <-}\StringTok{ }\NormalTok{df_centr_betweenness }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{top_n}\NormalTok{(}\DecValTok{5}\NormalTok{) }\OperatorTok{%>%}\StringTok{  }\CommentTok{# highest values}
\StringTok{  }\KeywordTok{arrange}\NormalTok{(}\OperatorTok{-}\NormalTok{centr_betweenness)}

\NormalTok{top_}\DecValTok{5}\NormalTok{_betweenness}
\end{Highlighting}
\end{Shaded}
...
Auch die Betweenness-Zentralität gibt als zentralsten Knoten den Knoten
Nummer 9 zurück. Betweenness misst das Ausmaß, in dem ein Knoten auf
kürzesten Pfaden zwischen anderen Knoten im Graphen positioniert ist.
Somit wird in dieser Teilmenge des Datensatzes der Knoten 9 und damit
das Produkt Nummer 9 als am meisten mit anderen Produkten gekaufte
Produkt verstanden.\\

Um die Fragestellung \emph{Welche Produkte werden hauptsächlich
eigenständig gekauft?} zu beantworten, werden im Folgenden die Top 5 der
Knoten ausgegeben, welche die wenigsten Kanten zu Nachbarknoten haben.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lowest_}\DecValTok{5}\NormalTok{_degree <-}\StringTok{ }\NormalTok{df_centr_degree }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{top_n}\NormalTok{(}\OperatorTok{-}\DecValTok{5}\NormalTok{)  }\CommentTok{# lowest values}
  
\NormalTok{lowest_}\DecValTok{5}\NormalTok{_degree}
\end{Highlighting}
\end{Shaded}
